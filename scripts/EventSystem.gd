...
func _process(delta:float)->void:
	...
	_timer+=delta
	if _timer>=_next:
		_timer=0.0; _next=randf_range(90.0,180.0)
		var ev=events[randi()%events.size()]; _active=true
		var sm=get_tree().current_scene.get_node_or_null("SpawnManager")
		if sm and sm.has_method("start_event"): sm.start_event(ev.bonus,ev.dur,ev.msg)
