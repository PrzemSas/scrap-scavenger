...
func _build_main_forge() -> void:
	...
	# Emissive forge opening (front face of base)
	_emit(Vector3(0, 0.92, -4.77), Vector3(1.80, 0.78, 0.06),
		  Color(1.0, 0.52, 0.08), 4.0)
	# Emissive bottom edge
	_emit(Vector3(0, 0.53, -4.77), Vector3(2.00, 0.06, 0.06),
		  Color(1.0, 0.30, 0.04), 2.5)
	# Emissive hot floor inside
	_emit(Vector3(0, 0.01, -6.0), Vector3(2.6, 0.05, 1.8),
		  Color(1.0, 0.26, 0.04), 1.8)

	# Spotlight beaming from forge mouth
...
func _build_braziers() -> void:
	...
	for pos in PIT_POSITIONS:
		# pedestal (narrows slightly going up)
		_cyl(pos + Vector3(0, 0.22, 0), 0.60, 0.70, 0.44, stone)
		# bowl rim (widens going up — cradles the fire)
