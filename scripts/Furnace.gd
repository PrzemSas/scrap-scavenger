...
func _uq()->void:
	var mx:=GameManager.get_smelt_queue_max()
	queue_l.text="QUEUE: %d/%d"%[_q.size(),mx]
	GameManager.smelt_queue_changed.emit(_q.size())
func load_mat(idx:int)->void:
	if _q.size()>=GameManager.get_smelt_queue_max(): return
