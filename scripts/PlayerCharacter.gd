...
# Base positions (must match .tscn)
const _BY  := 0.60
const _HY  := 1.42
const _HTY := 1.52
const _HFY := 1.38
...
@onready var _shoulder_l:   MeshInstance3D = $ShoulderL
@onready var _shoulder_r:   MeshInstance3D = $ShoulderR
@onready var _chest_plate:  MeshInstance3D = $ChestPlate
@onready var _cyan_stripe:  MeshInstance3D = $CyanStripe
@onready var _glove_l:      MeshInstance3D = $GloveL
@onready var _glove_r:      MeshInstance3D = $GloveR
@onready var _knee_l:       MeshInstance3D = $KneeL
@onready var _knee_r:       MeshInstance3D = $KneeR
...
func _animate(delta: float) -> void:
	...
	# Shoulders and chest armor follow body bob
	_shoulder_l.position  = Vector3(_SLX, _SY  + body_bob, 0)
	_shoulder_r.position  = Vector3(_SRX, _SY  + body_bob, 0)
	_chest_plate.position = Vector3(0,    _CPY + body_bob, _CPZ)
	_cyan_stripe.position = Vector3(0,    _CSY + body_bob, _CSZ)
	...
	# Knee pads track with legs
	_knee_l.position = Vector3(_KLX, _KY, -sc * leg_amp * b + _KZO)
	_knee_r.position = Vector3(_KRX, _KY,  sc * leg_amp * b + _KZO)
