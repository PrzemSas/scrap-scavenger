...
func setup(data:Dictionary)->void:
	...
	var mi:MeshInstance3D=$MeshInstance3D; var cs:CollisionShape3D=$CollisionShape3D
	...
	match data.get("id","can"):
		...
		"chip":
			...
			var s:=BoxShape3D.new(); s.size=Vector3(0.25,0.1,0.2); cs.shape=s
		"coil":
			var m:=TorusMesh.new(); m.inner_radius=0.04; m.outer_radius=0.15; mi.mesh=m
			var s:=SphereShape3D.new(); s.radius=0.18; cs.shape=s
		"motor":
		...
		"gear":
			...
			var s:=CylinderShape3D.new(); s.radius=0.2; s.height=0.12; cs.shape=s
		"gold":
		...
		"crystal":
			...
			var s:=BoxShape3D.new(); s.size=Vector3(0.25,0.45,0.25); cs.shape=s
		_:
...
func _process(delta:float)->void:
	...
	_bob+=delta; position.y=_iy+0.5+sin(_bob*2)*0.1
...
func _on_input_event(_c:Node,ev:InputEvent,_p:Vector3,_n:Vector3,_i:int)->void:
	if ev is InputEventMouseButton and ev.pressed and ev.button_index==MOUSE_BUTTON_LEFT: collect()
...
func collect()->void:
	...
	_collected=true; var pos:=global_position
	...
	get_tree().current_scene.add_child(sp); get_tree().create_timer(1.0).timeout.connect(sp.queue_free)
