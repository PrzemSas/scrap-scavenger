...
func _ready() -> void:
	...
	GameManager.upgrade_purchased.connect(func(_id):
		_active = GameManager.upgrades.get("lucky_find", 0) >= 3
		visible = _active
	)
...
func _process(_delta: float) -> void:
	...
	for ch in sm.get_children():
		if ch is Area3D and "scrap_data" in ch:
			...
			if r >= 2 and r > best_r:
				...
				best = ch
	if best == null:
		info.text = "No rare nearby"
		arrow.text = "—"
		return
	var cam = get_tree().current_scene.get_node_or_null("Camera3D")
	if not cam:
		return
	var dir = best.global_position - cam.global_position
	dir.y = 0
	...
	if idx < 0:
		idx += 8
	arrow.text = arrows[idx]
	var dist = dir.length()
	var names = ["", "", "RARE", "LEGEND"]
	info.text = "%s %.0fm" % [names[best_r], dist]
	var colors = [Color.WHITE, Color.WHITE, Color("#00e5ff"), Color("#FFD700")]
	arrow.add_theme_color_override("font_color", colors[best_r])
	info.add_theme_color_override("font_color", colors[best_r])
